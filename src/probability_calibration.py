...
class probability_calibration():
    '''
    The purpose of this class is to convert a neural network outcome for binary classification to probability in non-parameteric manner
    ...
    def histModel(self,nbins:int):
        ...
        for i in range(nbins):
            ...
            alpha_b = N1byB*pb
            beta_b = N1byB*(1-pb)
            m_b = np.sum(self.gt*(self.pred>=b_edges[i])*(self.pred<b_edges[i+1]))
            n_b = Nb - m_b
            p_sum = np.sum(self.pred*(self.pred>=b_edges[i])*(self.pred<b_edges[i+1]))
            p0 = (p_sum+pb)/(m_b+1)
            ...
            for b in bins_sel:
                ...
                if len(freq)>=2:
                    for kk in range(len(freq)):
                        if (self.infer[i]>=b_edges[kk]) and (self.infer[i]<b_edges[kk+1]):
                            pp = freq[kk]
                            break
